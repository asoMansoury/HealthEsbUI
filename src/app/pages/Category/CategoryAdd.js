...
import { Is_added } from '../_redux/Actions/categoryActions';
import {GeneralParamterInsertApi} from '../commonConstants/ApiConstants';
...
export class CategoryAdd extends React.Component {
    ...
    notifyError = (title) => toast('عملیات افزودن دسته بندی ' + title + ' با خطا مواجه شد.', { duration: toastConfig.duration, style: toastConfig.errorStyle });
    notifyInfo = (title) => toast('در حال افزودن دسته بندی  ' + title + ' ...', { duration: toastConfig.duration, style: toastConfig.infoStyle });
    ...
    save() {
        ...
        axios.post(GeneralParamterInsertApi, { Code: '0003', Title: title, Description: desc })
            .then(res => {
...
const mapStateToProps = (state => {
    return {
        Show_Hide_Add: state.category.Show_Hide_Add,
        Is_Added: state.category.Is_Added
    };
});
