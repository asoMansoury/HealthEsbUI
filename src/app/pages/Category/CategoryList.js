  ...
  return (
    ...
    >
      ...
      <div className={classes.actions}>
        {numSelected > 0 ? (
          <CategoryModalDeleteGroup selected={props.selected} />
        )
          :
          (<Button onClick={addCategoryShow} className='btn-height2 create-btn' variant="info">افزودن دسته بندی</Button>)}
      </div>
    </Toolbar>
  );
};
                      ...
                      <>
                        ...
                        >
                          ...
                          <TableCell align="center">
                            <Skeleton duration={1} style={{ width: '100%', display: isLoading ? 'block' : 'none', height: '20px' }} />
                            <div style={{ display: !isLoading ? 'block' : 'none' }}>
                              {row.description === '' ? <span className='pointer label label-lg label-light-danger label-inline btn-height'>فاقد توضیحات</span> : <CategoryModalEdit dbid={row.id} title="مشاهده توضیحات" headerTitle="دسته بند" name={row.title} text={row.description} />}
                            </div>
                          </TableCell>
                          <TableCell align="center">
                            <div style={{ display: !isLoading ? 'block' : 'none' }}>
...
const mapStateToProps = (state => {
  return {
    ...
    Is_Deleted_One: state.category.Is_Deleted_One,
    Is_Deleted_Group: state.category.Is_Deleted_Group,
